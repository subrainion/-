...
{
    // Обновленный SQL запрос, который выбирает только номер карточки и имя с фамилией пользователя
    ...
    while (query.next()) {
        ui->tableWidget->insertRow(row);
        ui->tableWidget->setItem(row, 0, new QTableWidgetItem(query.value(0).toString()));  // Номер карточки
        ui->tableWidget->setItem(row, 1, new QTableWidgetItem(query.value(1).toString()));  // Имя Фамилия
        ++row;
    }
...
void MainWindow::on_AddBook_clicked()
{
    ...
    if (library->addBook(title, author, category, availabilityCount)) {
...
void MainWindow::on_DeleteBook_clicked()
{
    ...
    if (library->deleteBook(title, author)) {
...
void MainWindow::on_GiveBook_clicked()
{
    ...
    if (library->issueBook(title, author, cardNumber)) {
...
{
    // Получаем номер библиотечного билета
    QString libraryCardNumber = QInputDialog::getText(this, "Вернуть книгу", "Введите номер библиотечного билета:");
    ...
    if (library->returnBook(libraryCardNumber, title, author)) {
...
void MainWindow::on_AddUser_clicked()
{
    ...
    QString cardNumber = QInputDialog::getText(this, "Добавить пользователя", "Введите номер библиотеки:");

    if (library->addUser(firstName, lastName, cardNumber)) {
...
{
    QString cardNumber = QInputDialog::getText(this, "Удалить пользователя", "Введите номер библиотеки для удаления:");
    if (library->deleteUser(cardNumber)) {
